...
function getPasswords() {
  ...
  loadinglabel.innerHTML = gettranslate("search_notes")+"...";
  var xhr = new XMLHttpRequest();
  var url = host+"/iSenhasBuscarNotasV4";
  if(development) url = host+"/iSenhasBuscarNotasV4DEV";
  ...
  xhr.onreadystatechange = async function () {
    if (xhr.readyState === 4 && xhr.status === 200) {
      ...
      if (records.length == 0) {
        ...
        todasSenhas = records;
        loadinglabel.innerHTML = gettranslate("search_notes")+": " + todasSenhas.length;
        var permission = getCookie("permission");
        if(permission == "ADMIN" || permission == "USER"){
          getUsers();
        } else {
      ...
      loadinglabel.innerHTML = gettranslate("search_notes_error")+": " + xhr.status;
    }
  };
...
function getShared() {
  ...
  xhr.send(""+JSON.stringify({"type":"note"}));
}
...
async function sair() {
  ...
  document.cookie = "tokenit=; expires=Thu, 01 Jan 1970 00:00:00 UTC; path=/;";
          ...
          $('#table-body').append(`
                  ...
                  <td class= "info" id="name${i + 1}">
                    <div style="display: flex;">   
                      ...
                      <img style="height:24px;margin-right:10px;" alt="Qries" src="../images/iconsmart/note.png" data-toggle="modal" data-target="#exampleModal" data-whatever="@fat" onClick= "copy(${i + 1})"/>
                    <div>
                    <div>${named}</div>
                    ...
                    <td class= "info" id="pass" onclick="select(${i+1})"><textarea rows="3" id="password" class="passwordRow" type="password" style="width:100%" readonly>${pass}</textarea></td>
...
function copy(id){
  ...
  var td = tr[id].getElementsByTagName("td")[1];
  var tdimage = tr[id].getElementsByTagName("td")[2];

  if(td){
    var p = td.getElementsByTagName("textarea")[0];
...
function select(id){
  ...
  if(td){
    if(td.className == "selected"){
      ...
      var recordName = tr[id].getElementsByTagName("td")[4].innerHTML;
      var recordChangeTag = tr[id].getElementsByTagName("td")[5].innerHTML;
...
function buttonadicionar(){
  ...
  if(modal.find('#notaadd').val().length == 0){
    document.getElementById("notaaddlabel").style.color = "#F4364C"
    return;
  } else {
    document.getElementById("notaaddlabel").style.color = "white"
  }
  modal.modal("hide")
  loading.style.display = "block";
  limiter.style.display = "none";
...
async function addNota() {
  ...
  loadinglabel.innerHTML = gettranslate("add_note")+"...";
  var xhr = new XMLHttpRequest();
  var url = host+"/iSenhasAddNotaV4";
  if(development) url = host+"/iSenhasAddNotaV4DEV";
  xhr.open("POST", url, true);
  xhr.setRequestHeader("Content-Type", "application/json");
  xhr.setRequestHeader('Access-Control-Allow-Origin', '*');
  xhr.setRequestHeader('authorization', token);
  ...
  xhr.onreadystatechange = function () {
    if (xhr.readyState === 4 && xhr.status === 200) {
      ...
      spinner.style.display = "none";
      var language = navigator.language || navigator.userLanguage;
      loadinglabel.innerHTML = gettranslate("add_note_error")+": " + xhr.status;
    }
  };
  var modal = $("#addModal")
  ...
  let myPassword = modal.find("#notaadd").val();
...
function buttonremove(){
  ...
  if(selectedRows.length==0){
    loadinglabel.innerHTML = gettranslate("no_selection");
    setTimeout(() => {
      loading.style.display = "none";
...
function excluirSenhas() {
  ...
  loadinglabel.innerHTML = gettranslate("remove_note")+": "+enviado;
  var xhr = new XMLHttpRequest();
  ...
  xhr.onreadystatechange = function () {
    if (xhr.readyState === 4 && xhr.status === 200) {
      ...
      spinner.style.display = "none";
      var language = navigator.language || navigator.userLanguage;
      loadinglabel.innerHTML = gettranslate("remove_note_error")+": " + xhr.status;
    }
  };
  var operations = [];
  var forvalue = 200;
...
function edit(id){
  ...
  if(password.fields.admin){
    ...
    modal.find('#notaedit').val(todasSenhas[id-1].fields.password.value)
    let translator = gettranslate("favorite_note")
    if(password.sharedDesc == null){
...
function buttoneditar(){
  ...
  if(modal.find('#notaedit').val().length == 0){
    document.getElementById("notaeditlabel").style.color = "#F4364C"
    return;
  } else {
    document.getElementById("notaeditlabel").style.color = "white"
  }
  modal.modal("hide")
  loading.style.display = "block";
...
async function editSenha() {
  ...
  loadinglabel.innerHTML = gettranslate("update_note")+"...";
  var xhr = new XMLHttpRequest();
  ...
  xhr.onreadystatechange = function () {
    if (xhr.readyState === 4 && xhr.status === 200) {
      ...
      spinner.style.display = "none";
      var language = navigator.language || navigator.userLanguage;
      loadinglabel.innerHTML = gettranslate("update_note_error")+ ": " + xhr.status;
    }
  };
  var modal = $("#editarModal")
  ...
  var myPassword = modal.find('#notaedit').val()
  if(isExtreme){
    myPassword = await encryptExtreme(myPassword)
  }
...
function translate(){
  ...
  document.getElementById("loadinglabel").innerHTML =  gettranslate("notas_loading");
  document.getElementById("nametitle").innerHTML = gettranslate("name");
  document.getElementById("passwordtitle").innerHTML = gettranslate("note");
  document.getElementById("copytitle").innerHTML = gettranslate("copy");
    document.getElementById("search").placeholder = gettranslate("note_search");
    ...
    document.getElementById("title").innerHTML = gettranslate("notes");
    ...
    document.getElementById("addModalLabel").innerHTML = gettranslate("new_note")
    document.getElementById("nomeaddlabel").innerHTML = gettranslate("name");
    document.getElementById("notaaddlabel").innerHTML = gettranslate("note");
    document.getElementById("nomeadd").placeholder = gettranslate("ex_note");
    document.getElementById("notaadd").placeholder = gettranslate("ex_note_desc");
    ...
    document.getElementById("excluirModalLabel").innerHTML = gettranslate("remove_notes");
    document.getElementById("removedesc").innerHTML = gettranslate("remove_notes_desc");
    ...
    document.getElementById("editModalLabel").innerHTML = gettranslate("edit_note");
    document.getElementById("nomeeditlabel").innerHTML = gettranslate("name");
    document.getElementById("notaeditlabel").innerHTML = gettranslate("note");
    document.getElementById("nomeedit").placeholder = gettranslate("ex_note");
    document.getElementById("notaedit").placeholder = gettranslate("ex_note_desc");
...
function atualizarSenha() {
  ...
  xhr.onreadystatechange = function () {
    if (xhr.readyState === 4 && xhr.status === 200) {
      limiter.style.display = "none";
      loading.style.display = "block";
      let difference = passwordPermissions.filter(x => !oldPermissions.includes(x));
      getPasswords();
    } else if (xhr.readyState === 4) {
      spinner.style.display = "none";
      var language = navigator.language || navigator.userLanguage;
